...
def getBARTSimilarity(Text1, Text2, tokenizer):
...
def DATASET_AttentionOnChunks(file1, file2, threshold, tokenizer, nlp, chunksize):
    ...
    for a in range(1, chunks1size):
        for b in range(1, chunks2size):
            res = getBARTSimilarity(chunks1[a], chunks2[b], tokenizer)
            if res > threshold:
               if res > Matrix[b][a]:
                  Matrix[b][a] = res
               if res < 0.1:
    ...
    for a in range(1, size1):
        ...
        if SVO1[a] == 1: 
            ...
            for b in range(1, size2):
                ...
                if SVO2[b] == 1: 
                    Text2 = Sent2[b]
                    res = getBARTSimilarity(Text1, Text2, tokenizer)
                    if res > threshold:
                        if res > Matrix[a][b]:
                             Matrix[a][b] = res
                        if res < 0.1:
...
res = getBARTSimilarity("Hello world", "Hello world", tokenizer)
print(res)
x = tokenizer("Hello world")["input_ids"]
print(x)
y = tokenizer("Hello world")["input_ids"]
print(y)
