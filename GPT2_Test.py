...
def getGPT2Similarity(Text1, Text2, tokenizer):
...
def DATASET_AttentionOnChunks(file1, file2, threshold, tokenizer, nlp, chunksize):
    ...
    for i in range(1, chunks1size):
        for k in range(1, chunks2size):
            res = getGPT2Similarity(chunks1[i], chunks2[k], tokenizer)
            if res > threshold:
                if res > Matrix[k][i]:
                    Matrix[k][i] = res
                if res < 0.1:
...
def DATASET_AttentionOnSentencesGPT2(file1, file2, threshold, tokenizer, nlp):
    filea = file1
    fileb = file2
    file1 ='S_'+ file1;
    file2 ='S_'+ file2;
    ...
    for a in range(1, size1):
        ...
        if SVO1[a] == 1: 
            for b in range(1, size2):
                # check if the sentence contains verb if not discard         
                if SVO2[b] == 1: 
                    res = getGPT2Similarity(Sent1[a], Sent2[b], tokenizer)
                    if res > threshold:
                        if res > Matrix[a][b]:
                             Matrix[a][b] = res
                        if res < 0.1:
    ...
    for a in range(1, size1):
        ...
        if SVO1[a] == 1: 
            ...
            for b in range(1, size2):
                ...
                if SVO2[b] == 1: 
                    Text2 = Sent2[b]
                    res = getGPT2Similarity(Text1, Text2, tokenizer)
                    if res > threshold:
                        if res > Matrix[a][b]:
                             Matrix[a][b] = res
                        if res < 0.1:
...
# '2022 Russian invasion of Ukraine.txt'
...
text1 = "Replace me by any text you'd like for attention"
text2 = "This is a random sentence"

Sim = getGPT2Similarity(text1, text2, tokenizer)
print(Sim)
